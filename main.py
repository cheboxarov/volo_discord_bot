...
if __name__ == "__main__":
    ...
    async def connect(ctx: discord.context.ApplicationContext):
        ...
        try:
            ...
            await ctx.respond(f"Successfully connected to the voice channel!", ephemeral=False)
    ...
    async def ink(ctx: discord.context.ApplicationContext):
        ...
        if not bot.guild_to_helper.get(ctx.guild_id, None):
            await ctx.respond(f"The bot is not connected to a voice channel. Please use {connect_text} to connect.", ephemeral=True)
            return
        if bot.guild_is_recording.get(ctx.guild_id, False):
            await ctx.respond("Recording is already in progress.", ephemeral=True)
            return
        bot.start_recording(ctx)
        await ctx.respond("Recording started!", ephemeral=False)
    ...
    async def update_user_map(ctx: discord.context.ApplicationContext):
        if bot.guild_is_recording.get(ctx.guild_id, False):
